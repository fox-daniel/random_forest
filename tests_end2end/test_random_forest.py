...
@pytest.mark.parametrize('num_points, classes, xbound, ybound, zbound, max_depth, min_node_size, min_loss, expected',
	[
	(np.full(8,1000), [0,0,0,0,0,0,0,1], .5, .5, .5, 30, 1, 0, [1, 1, 0]),
	(np.full(8,1000), [1,0,0,0,0,0,0,1], .5, .5, .5, 30, 1, 0, [1, 1, 0])
	])
def test_octavo(num_points, classes, xbound, ybound, zbound, max_depth, min_node_size, min_loss, expected):
	...
	forest = random_forest.grow_random_forest(X, y, num_trees = 20, max_features = 2, min_node_size = 1)
...
@pytest.mark.parametrize('num_points, dim, max_features, expected, precision_bound',
	[
	(1000, 2, 2, [1, 1, 0], .01),
	(1000, 3, 2, [1, 1, 0], .01),
	(1000, 4, 2, [1, 1, 0], .01),
